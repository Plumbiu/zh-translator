import { getLocalName, localeDisplayName, SelectAutoDetectValue, supportsLanguages } from "../constants"
...
export const SelectTargetLanguageHtml =
  supportsLanguages.map(language => {
    const label = getLocalName(language)
    const selected = label === localeDisplayName
    return `<option ${selected ? 'selected' : ''} value="${language}">${label}</option>`
  }).join('')

export const SelectSourceLanguageHtml = `<option selected value="${SelectAutoDetectValue}">Auto</option>` + supportsLanguages.map(language => {
  const label = getLocalName(language)
